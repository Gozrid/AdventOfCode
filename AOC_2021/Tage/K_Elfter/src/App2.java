...
public class App2 {
    public static void main(String[] args) throws Exception {
        ...
        int step = 0;
        for (int i = 0; i < 500; i++) {     //Besser while schleife, aber so geht es auch
            step(map);
            if (flashing(map) == (map.length * map[0].length)) {
                step = i;
                break;
            }
        }
        System.out.println(step+1);     //i offset von 1 korrigieren!
        input.close();
    }   
