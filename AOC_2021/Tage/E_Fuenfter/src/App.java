...
public class App {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            if (X != 0) {
                for (int j = 0; j <= X; j++) {
                    map[Integer.min(input.get(i)[0], input.get(i)[2]) + j] [input.get(i)[1]] += 1;
                }
            }else{
                for (int j = 0; j <= Y; j++) {
                    map[input.get(i)[0]] [Integer.min(input.get(i)[1], input.get(i)[3]) + j] += 1;
                }
            }
        }
        //drawMap(map);
    ...
    private static void filterInput(ArrayList <int[]> input) {
        for (int i = input.size()-1; i >= 0; i--) {
            if (input.get(i)[0] != input.get(i)[2] && input.get(i)[1] != input.get(i)[3]) input.remove(i);
        }
    }
